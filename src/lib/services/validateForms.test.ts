...
describe('validDateEventData', () => {
	...
	it('error: no startdate for event', () => {
		...
		expect(notificationStore.addToast).toHaveBeenCalledWith('error', 'Mandatory start date is empty');
	});
	...
	it('error: no starttime for event', () => {
		...
		// @ts-expect-error - Testing with incomplete event object
		validateEventData(event);
		expect(notificationStore.addToast).toHaveBeenCalledWith('error', 'Mandatory start time is empty');
	});
	...
	it('error: startdate before today', () => {
		...
		// @ts-expect-error - Testing with incomplete event object
		validateEventData(event);
		expect(notificationStore.addToast).toHaveBeenCalledWith('error', 'The start date cannot be in the past');
	});
	...
	it('error: endDateTime before startDateTime based on enddate', () => {
		...
		// @ts-expect-error - Testing with incomplete event object
		validateEventData(event);
		expect(notificationStore.addToast).toHaveBeenCalledWith('error', 'The event end cannot be before the event start');
	});
	...
	it('warning: endDateTime === startDateTime ', () => {
		...
		// @ts-expect-error - Testing with incomplete event object
		validateEventData(event);
		expect(notificationStore.addToast).toHaveBeenCalledWith('warning', 'Both start and end date-time are the same');
	});
